...
class AddressBar: UIView, UITextFieldDelegate {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.clear//Colors.whiteBg
...
//                make.right.equalTo(self).offset(-8)
        ...
        addressField = SharedTextField().then { [unowned self] in
            $0.placeholder = "Search or type web address"
...
//            }
            $0.setImage(UIImage(named: "noun_reload")?.withRenderingMode(.alwaysTemplate), for: .normal)
            $0.backgroundColor = UIColor.clear
...
//				make.right.equalTo(self).offset(-8)
    ...
    func checkForLocalhost() {
        if let address = addressField?.text, address.contains("file:///") {
            addressField?.text = ""
        }
    }
...
//        self.viewSerach?.backgroundColor = isPrivate ? Colors.darkBg : UIColor.white//UIColor(red: 239/255, green: 241/255, blue: 245/255, alpha: 1.0)
